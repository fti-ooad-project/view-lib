...
DEVICE QueuePixel Scene::traceRayQueued( QueueRay const &qray , QueueRay *rqueue , int index , int SAMPLES_COUNT ) const
{
	Collision col;
	Material mat;
	{
		...
		if( !success )
		{
			return{ qray._target_pixel , qray._color_k & (f3( 0.2f , 0.23f , 0.46f ) + qray._v.z() * f3( 1.9f , 1.6f , 1.5f )) };
		}
		if( mat._emit )
		{
			return{ qray._target_pixel , qray._color_k & mat._color };
		}
	}
	f3 out( 0.0f );
	{
		...
		if( mat._transperency < 1.0f )
		{
			...
			for( int i = 0; i < SAMPLES_COUNT; i++ )
			{
				...
				temp_ray._v = v;
				rqueue[ index * SAMPLES_COUNT * 2 + i ] = temp_ray;
			}
		}
		f3 refr( 0.0f );
		if( mat._transperency > 0.0f )
		{
			...
			for( int i = 0; i < SAMPLES_COUNT; i++ )
			{
				...
				temp_ray._v = v;
				rqueue[ index * SAMPLES_COUNT * 2 + SAMPLES_COUNT + i ] = temp_ray;
			}
		}
	}
	return{ -1 , f3( 0.0f ) };
